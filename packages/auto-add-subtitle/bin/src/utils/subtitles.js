...
function merge(primaryLines, secondLines) {
  var _a = [parseMaxTime(primaryLines), parseMaxSequence(primaryLines)],
    maxTime = _a[0],
    maxSequence = _a[1];
  return primaryLines.concat(
    secondLines.map(function (line) {
      ...
      if (timelineReg.test(line)) {
        return line
          .split(' --> ')
          .map(function (time) {
            return formatTime(parseTime(time) + maxTime);
          })
          .join(' --> ');
      }
      return line;
      ...
      switch (_a.label) {
        case 0:
          return [
            4 /*yield*/,
            Promise.all(
              srtFiles.map(function (file) {
                return fs_1.readFile(file).then(function (buf) {
                  return buf.toString();
                });
              }),
            ),
